...
def load_module():
    ...
    path = Path(__file__).parents[1] / "functions/actions/elevenlabs_tts_enhanced/main.py"
    spec = importlib.util.spec_from_file_location("elevenlabs_tts_enhanced", path)
        ...
        "messages": [
            {
                ...
                "content": (
                    ...
                    "🎧 **Audio ready, Donovan.** Open the attached MP3 to preview it; "
                    "click its filename or download control to save it.\n\n"
                    "[Download MP3](/api/v1/files/file-1/content?attachment=true)"
                ),
            }
        ]
    }
...
def test_binary_file_content_explains_the_native_preview_and_download_controls():
    assert tts.file_content_summary("Donovan", "sample.mp3") == (
        "This is an MP3 audio file generated with the ElevenLabs voice **Donovan**.\n\n"
...
async def test_create_file_uses_current_async_openwebui_file_api(monkeypatch):
    class Files:
        ...
        async def insert_new_file(user_id, form):
            ...
            assert "Preview" in form["data"]["content"]
            assert form["meta"]["data"]["voice"] == "Donovan"
    ...
    assert await tts.Action.create_file(
        "test.mp3", b"audio", {"id": "user-1"}, "Donovan"
    )


@pytest.mark.asyncio
async def test_action_persists_a_native_file_attachment_and_updates_the_message(monkeypatch):
    action = tts.Action()
    action.valves.ELEVENLABS_API_KEY = "configured"
    emitted = []
    ...
    async def voices():
        return {"Donovan": "voice-id"}, {"Donovan": "Clear"}

    async def generate(voice_id, text):
        assert voice_id == "voice-id"
        assert text == "Narrate this."
        return b"mp3-bytes"

    async def save(filename, content, user, voice_name):
        assert filename.endswith(".mp3")
        assert content == b"mp3-bytes"
        assert user == {"id": "user-1"}
        assert voice_name == "Donovan"
        return "file-1"

    monkeypatch.setattr(action, "voice_options", voices)
    monkeypatch.setattr(action, "generate_audio", generate)
    monkeypatch.setattr(action, "create_file", save)

    result = await action.action(
        {
            "id": "assistant-1",
            "messages": [{"role": "assistant", "content": "Narrate this."}],
        },
        __user__={"id": "user-1"},
    ...
    assert any(event["type"] == "files" for event in emitted)
    files_event = next(event for event in emitted if event["type"] == "files")
    assert files_event["data"]["files"] == [
        tts.file_attachment("file-1", files_event["data"]["files"][0]["name"], 9)
    ]
    assert result["messages"][0]["id"] == "assistant-1"
    assert "Audio ready" in result["messages"][0]["content"]
