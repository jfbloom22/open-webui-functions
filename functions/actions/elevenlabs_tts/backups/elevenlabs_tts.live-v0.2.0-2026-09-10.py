...
class Action:
    ...
    async def fetch_available_voices(self) -> tuple[str, dict[str, str]]:
        ...
        try:
            ...
            return (
                f"Sorry, couldn't fetch available voices at the moment (HTTP {e.status}).",
                {},
            )
            ...
            return (
                "Sorry, couldn't fetch available voices at the moment (network error).",
                {},
            )
    ...
    async def action(
        ...
        try:
            ...
            if (
                not self.valves.ELEVENLABS_API_KEY
                or not self.valves.ELEVENLABS_API_KEY.strip()
            ):
                raise ValueError(
                    "ElevenLabs API key is not configured. Please set it in the function settings."
                )
            ...
            if not __event_call__:
                raise ValueError(
                    "Action requires user interaction but event_call is not available"
                )
            ...
            if default_voice not in self.voice_id_cache:
                voice_names_lower = {
                    name.lower(): name for name in self.voice_id_cache.keys()
                }
                if default_voice.lower() in voice_names_lower:
                    ...
                    if DEBUG:
                        print(
                            f"Debug: Default voice '{self.valves.DEFAULT_VOICE}' not found. Using '{default_voice}' instead."
                        )
            ...
            if DEBUG:
                ...
                print(
                    f"Debug: Available voices: {list(self.voice_id_cache.keys())[:10]}"
                )
            ...
            if not assistant_message or not assistant_message.strip():
                raise ValueError(
                    "No assistant message content found to convert to speech"
                )
...
"""
                                ...
                                return {
                                    "content": f"Audio generated successfully using ElevenLabs voice **{selected_voice_name}**. Download: {file_url}"
                                }
                        else:
                        ...
                        raise ValueError(
                            f"Unexpected API response: {response.status} - {response_text}"
                        )
